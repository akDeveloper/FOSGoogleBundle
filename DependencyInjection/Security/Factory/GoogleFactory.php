...
class GoogleFactory extends AbstractFactory
{
  ...
  protected function createAuthProvider( ContainerBuilder $container, $id, $config, $userProviderId )
  {
    ...
    $definition = $container->setDefinition( $authProviderId, new DefinitionDecorator( 'fos_google.auth') )->replaceArgument( 0, $id );

    // with user provider
    if ( isset( $config['provider'] ) )
    {
      $definition->addArgument( new Reference( $userProviderId) )->addArgument( new Reference( 'security.user_checker') )->addArgument( $config['create_user_if_not_exists'] );
    }
